...
function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('procash_simple_user');
    if (savedUser) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const records = snapshot.docs.map(doc => doc.data() as CashRecord);
      setHistory(records);
    }, (error) => {
  ...
  const sendEmailReport = (date: string, records: CashRecord[], dayTotal: number) => {
    ...
    let body = `Pro Cash Calculator - Daily Report\nDate: ${date}\n\n`;
    ...
    body += `\n\nGenerated by Pro Cash Calculator`;
  ...
  const handleAICommand = useCallback((cmd: { action: string; label?: string; count?: number }) => {
    ...
    if (cmd.label) {
      ...
      let searchLabel = cmd.label.toLowerCase();
      
      // Fallback mapping for common terms if the model ignores instructions
      const commonMappings: Record<string, string> = {
        'hundreds': '100', 'hundred': '100',
        'fifties': '50', 'fifty': '50',
        'twenties': '20', 'twenty': '20',
        'tens': '10', 'ten': '10',
        'fives': '5', 'five': '5',
        'ones': '1', 'one': '1',
        'quarters': '0.25', 'quarter': '0.25',
        'dimes': '0.10', 'dime': '0.10',
        'nickels': '0.05', 'nickel': '0.05',
        'pennies': '0.01', 'penny': '0.01'
      };
      
      if (commonMappings[searchLabel]) {
        searchLabel = commonMappings[searchLabel];
      }

      const searchStr = searchLabel.replace(/[^0-9.]/g, '');
      ...
      const denom = currency.denominations.find(d => {
        ...
        return (
          labelLower === searchLabel ||
          labelLower.includes(searchLabel) ||
      ...
      if (denom) {
        ...
        // Close assistant after successful command
        setTimeout(() => {
          setShowAssistant(false);
          setIsLiveActive(false);
        }, 1000);
  ...
  const saveRecord = async () => {
    ...
    try {
      await setDoc(doc(db, 'users', user.uid, 'records', recordId), record);
    } catch (error) {
  ...
  const handleLogout = () => {
    localStorage.removeItem('procash_simple_user');
    setUser(null);
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 relative">
      {!user && isAuthReady && <Login onLoginSuccess={setUser} />}
      ...
      <main className="flex-1 flex flex-col min-w-0 relative z-10">
        <header className="h-auto py-4 glass border-b border-slate-200 px-4 flex flex-col gap-4 shrink-0 relative z-20">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-extrabold text-slate-900 tracking-tight text-lg">Pro Cash <span className="text-indigo-600">Calculator</span></h1>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
          ...
          <aside className="space-y-6">
            ...
            <CurrencyConverter key={sessionKey} initialCurrency={currency.code} />
                    ...
                    <div className="flex items-center justify-between sticky top-0 bg-white/90 backdrop-blur-sm py-1 z-10">
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          ...
                          className="p-2 bg-blue-500 text-white hover:bg-blue-600 hover:scale-110 rounded-xl transition-all shadow-lg shadow-blue-100"
                          title="Email Report"
                        >
